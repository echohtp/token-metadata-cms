-- Database Functions for Token Metadata CMS
...
CREATE OR REPLACE FUNCTION validate_solana_address(address VARCHAR)
RETURNS BOOLEAN AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION validate_url(url VARCHAR)
RETURNS BOOLEAN AS $$
BEGIN
    ...
    IF NOT (url ~ '^https?://[^\s/$.?#].[^\s]*$') THEN
        RETURN false;
    END IF;
    
    -- Prevent suspicious URLs
    IF url ~* '(javascript:|data:|vbscript:|file:|ftp:)' THEN
        RETURN false;
    END IF;
...
CREATE OR REPLACE FUNCTION is_wallet_authorized(p_wallet_address VARCHAR)
RETURNS TABLE (
...
CREATE OR REPLACE FUNCTION get_token_by_mint(p_mint VARCHAR)
RETURNS TABLE (
    id BIGINT,
    mint VARCHAR,
    name VARCHAR,
...
CREATE OR REPLACE FUNCTION upsert_token_metadata(
    p_mint VARCHAR,
...
CREATE OR REPLACE FUNCTION delete_token_metadata(
    p_mint VARCHAR,
    p_wallet_address VARCHAR DEFAULT NULL
)
RETURNS BOOLEAN AS $$
...
CREATE OR REPLACE FUNCTION add_authorized_wallet(
...
CREATE OR REPLACE FUNCTION get_authorized_wallets()
RETURNS TABLE (
    id BIGINT,
